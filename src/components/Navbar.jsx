...
>
  ...
  <div className="flex flex-col">
    <span className="font-bold text-lg md:text-xl tracking-tight">KRISHNA</span>
    <span className="text-xs text-blue-700 md:text-sm">ENGINEERING COLLEGE</span>
    <span className="text-xs text-blue-600 font-medium md:text-sm">Managed By KPS GROUP</span>
          ...
          >
           <motion.a
  href="https://timer.addonforge.com/timer/1FAIpQLSeGCSTjQ1NPp99s8btnvjuJd8e_k8vPQWm5oQEHYNfmjCWHhA"
  target="_blank"
  rel="noopener noreferrer"
  whileHover={{
    scale: 1.05,
    boxShadow: "0 8px 20px -5px rgba(234, 88, 12, 0.5)", // orange shadow
  }}
  whileTap={{ scale: 0.95 }}
  className="bg-orange-600 text-white font-bold px-6 py-2.5 rounded-lg shadow-md hover:bg-orange-500 transition-colors duration-300 flex items-center space-x-2"
>
  <span>Super 20 Entrance Exam</span>
  <motion.span
    animate={{ x: [0, 4, 0] }}
              ...
              {navItems.map((item, index) => (
                <div key={index} className="border-b border-gray-100 last:border-b-0">
                  <div
                    ...
                    onClick={() =>
                      item.dropdown ? toggleMobileDropdown(item.name) : setIsOpen(false)
                    }
                  >
